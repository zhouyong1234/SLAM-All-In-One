...
KeyFrame::KeyFrame(double _time_stamp, int _index, const cv::Mat &_image_intensity, const pcl::PointCloud<PointType>::Ptr _cloud)
{
    ...
    {
        #pragma omp section
        computeWindowOrbPoint(); // 计算窗口内的orb特征，用于ransac pnp算法对回环关键帧和当前关键帧的匹配进行验证(回环验证需要)
        #pragma omp section
        computeWindowBriefPoint(); // 计算窗口内的brief特征，用于ransac pnp算法对回环关键帧和当前关键帧的匹配进行验证(回环验证需要)
        #pragma omp section
        computeSearchOrbPoint(); // 计算用于搜索的orb特征(相比于窗口内的特征多提取5倍的特征点数)(回环验证需要)
        #pragma omp section
        computeSearchBriefPoint(); // 计算用于搜索的brief特征(相比于窗口内的特征多提取5倍的特征点数)(回环验证需要)
        #pragma omp section
        computeBoWPoint(); // 计算用于bow检索需要的orb特征(回环检测需要)
    }
...
void KeyFrame::computeSearchOrbPoint()
{
    if (USE_ORB)
    {
        // ORB features
        vector<uchar> status;
        // 相比于先前接口computeWindowOrbPoint()多计算5倍的特征点数
...
void KeyFrame::computeSearchBriefPoint()
{
    if (USE_BRIEF)
    {
        // Corner feautres
        vector<uchar> status;
        // 相比于先前接口computeWindowBriefPoint()多计算5倍的特征点数
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
    ...
    if (USE_ORB)
    {
        // 使用OpenCV暴力匹配器BFMatcher对回环关键帧和当前关键帧进行特征匹配
        ...
        if ((int)good_matches.size() > MIN_LOOP_FEATURE_NUM)
        {
            ...
            // 利用ransac pnp方法计算成功配对的点，并记录到状态数组中
            ...
            // 当匹配的特征点数超过给定的阈值，并且这些特征点分布有效，则认为当前关键帧和回环关键帧匹配有效
                         ...
                         std::vector<uchar> &status)
{
    ...
    // 将当前关键帧特征点投影到回环关键帧图像上的方式进行ransac pnp求解
                        ...
                        vector<uchar>& out_status)
{
    ...
    for (size_t i = 0; i < in_point_2d_uv.size(); ++i)
    {
        ...
        if (abs(pi->x) < 0.01) // 如果特征点对应的雷达点的x坐标小于0，则将其输出状态置为0，后面会基于这个状态将对应的特征点删除掉
        {
            out_status[i] = 0;
        } 
        else // 否则，将输出状态置为1
        {
            out_status[i] = 1;
            // lidar -> camera // 将特征点对应的雷达点在相机坐标系下表达(相机坐标系z轴与雷达坐标系x轴重合)
            p_3d.x = -pi->y;
            p_3d.y = -pi->z;
            p_3d.z = pi->x;
            // normalize to projection plane // 对转换到相机坐标系下的雷达点进行归一化，相当于投影到了单位平面上
