...
struct Camera
{
  ...
  _Scalar c_u, c_v, f_u, f_v, b; //相机内参
...
{
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  _Scalar u_var_prime, v_var_prime;                 //特征方差
  Eigen::Matrix<_Scalar, 12, 12> Q_imu;             //imu方差
  Eigen::Matrix<_Scalar, 15, 15> initial_imu_covar; //初始时刻imu状态方差
};
...
struct featureTrackToResidualize
{
  ...
  std::vector<Vector2<_Scalar>,
              Eigen::aligned_allocator<Vector2<_Scalar>>>
      observations;
