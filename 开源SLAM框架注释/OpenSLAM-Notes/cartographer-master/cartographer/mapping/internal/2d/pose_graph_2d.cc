...
// 构造函数
// options                配置选项
// optimization_problem   2d优化问题
// thread_pool            线程池
// 初始化类 ConstraintBuilder2D 
...
// 析构函数
PoseGraph2D::~PoseGraph2D() {
  WaitForAllComputations();// 等待所有计算完成
...
// 添加一个新节点 
// constant_data      前端匹配的结果
// trajectory_id      轨迹id 
// insertion_submaps  需要插入的子地图
// optimized_pose     和全局地图间的位姿关系初始值
...
// 添加子地图时进行回环检测
// constant_data     前端匹配结果
// trajectory_id     轨迹id 
// insertion_submaps 插入子地图 
NodeId PoseGraph2D::AddNode(
    std::shared_ptr<const TrajectoryNode::Data> constant_data,
    const int trajectory_id,
    const std::vector<std::shared_ptr<const Submap2D>>& insertion_submaps) {
  // 优化位置,获得局部地图到全局地图的坐标变化关系
  const transform::Rigid3d optimized_pose(
...
void PoseGraph2D::AddImuData(const int trajectory_id,
  ...
  // 调用 AddWorkItem 函数，添加一个任务 任务内执行 optimization_problem_->AddImuData 函数
  AddWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
    absl::MutexLock locker(&mutex_);
    if (CanAddWorkItemModifying(trajectory_id)) {
	  // 实际上调用的是 	optimization_problem_->AddImuData 函数
...
void PoseGraph2D::AddOdometryData(const int trajectory_id,
  ...
  // 调用 AddWorkItem 函数，添加一个任务 任务内执行 optimization_problem_->AddOdometryData 函数
  AddWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
    absl::MutexLock locker(&mutex_);
    if (CanAddWorkItemModifying(trajectory_id)) {
...
void PoseGraph2D::AddFixedFramePoseData(
  ...
  // 调用 AddWorkItem 函数，添加一个任务 任务内执行 optimization_problem_->AddFixedFramePoseData 函数
  AddWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
    absl::MutexLock locker(&mutex_);
    if (CanAddWorkItemModifying(trajectory_id)) {
...
// 插入一个子地图时调用，计算子地图节点和其他子地图节点之间的约束关系
// node_id  节点id
// insertion_submaps 插入的子地图 
// newly_finished_submap 是否为新完成的子地图
WorkItem::Result PoseGraph2D::ComputeConstraintsForNode(
  ...
  std::set<NodeId> newly_finished_submap_node_ids;
  {
	...
	// 所有子地图id ，调用函数 InitializeGlobalSubmapPoses 将新的子地图插入优化问题中
...
void PoseGraph2D::DrainWorkQueue() {
  ...
  while (process_work_queue) {
    std::function<WorkItem::Result()> work_item;// 工作项
    {
      absl::MutexLock locker(&work_queue_mutex_);
	  // 当前工作项为空，则直接返回
  ...
  // 所有的约束添加完成了
  // 调用 WhenDone 函数，即又新建了一个任务线程，调用函数 HandleWorkQueue
