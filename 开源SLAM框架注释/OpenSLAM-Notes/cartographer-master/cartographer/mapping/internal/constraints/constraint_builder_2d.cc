...
// 构造函数，新建了两个任务
// finish_node_task_ 和 when_done_task_
...
void ConstraintBuilder2D::WhenDone(
  ...
  when_done_ = absl::make_unique<std::function<void(const Result&)>>(callback);// 赋值函数指针给callback
  CHECK(when_done_task_ != nullptr);
  // 添加任务，运行函数 RunWhenDoneCallback 
...
// WhenDone 函数中创建单独的线程调用该函数
void ConstraintBuilder2D::RunWhenDoneCallback() {
  Result result;
  std::unique_ptr<std::function<void(const Result&)>> callback;// 回调函数
  {
    absl::MutexLock locker(&mutex_);
    CHECK(when_done_ != nullptr);
	// 遍历所有的约束，放到result 中
