...
class ThreadPool : public ThreadPoolInterface {
 ...
 private:
  ...
  absl::Mutex mutex_;
  bool running_ GUARDED_BY(mutex_) = true;// 线程池运行状态
  std::vector<std::thread> pool_ GUARDED_BY(mutex_);// 线程向量
  std::deque<std::shared_ptr<Task>> task_queue_ GUARDED_BY(mutex_);// 任务向量
