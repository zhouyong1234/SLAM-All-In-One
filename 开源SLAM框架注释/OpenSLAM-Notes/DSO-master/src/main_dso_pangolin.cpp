...
int main( int argc, char** argv )
{
	...
	// new 一个fullSystem类，调用接口
	// 1 构造函数
	// 2 setGammaFunction 
	// 3 变量 linearizeOperation
	// 4 outputWrapper.push_back
	// 5 fullSystem->addActiveFrame(img, i);
	FullSystem* fullSystem = new FullSystem();
	fullSystem->setGammaFunction(reader->getPhotometricGamma());
	fullSystem->linearizeOperation = (playbackSpeed==0);

	// new 一个PangoliDSOViewer 类，调用接口
	// 1 构造函数 PangolinDSOViewer(wG[0],hG[0], false)
	// 2 viewer->run() 
