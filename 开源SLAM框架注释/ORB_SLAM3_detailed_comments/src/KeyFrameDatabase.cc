...
vector<KeyFrame *> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F, Map *pMap)
{
    ...
    // Search all keyframes that share a word with current frame
    {
        ...
        for (DBoW2::BowVector::const_iterator vit = F->mBowVec.begin(), vend = F->mBowVec.end(); vit != vend; vit++)
        {
            ...
            {
                KeyFrame *pKFi = *lit;
                //如果该关键帧的mnRelocQuery变量和当前帧的Id不同，则说明该关键帧还未与当前帧产生关联
                //因此给mnRelocQuery赋值，并初始化mnRelocWords变量，该变量表征相同单词的个数
                //并将该关键帧加入到上述list中
                ...
                //该关键帧和当前帧产生关联，并且mnRelocWords+1，这个值是关键帧的成员变量，由于相同的 
                //单词可能不止一个，该值可能会在后续的按单词遍历的过程中增加
                pKFi->mnRelocWords++;
            }
        }
    }
