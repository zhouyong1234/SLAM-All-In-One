...
bool ImuGpsLocalizer::ProcessImuData(const ImuDataPtr imu_data_ptr, State* fused_state, double gps_timestamp) {
    if (!initialized_) {
    ...
    // Predict.
    imu_processor_->Predict(state_.imu_data_ptr, imu_data_ptr, &state_, gps_timestamp);
...
bool ImuGpsLocalizer::ProcessGpsPositionData(const GpsPositionDataPtr gps_data_ptr, State* fused_state) {
    if (!initialized_) {
