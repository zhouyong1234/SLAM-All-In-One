...
void ImuProcessor::Predict(const ImuDataPtr last_imu, const ImuDataPtr cur_imu, State* state, double gps_timestamp) {
    // Time.
    ...
    delta_time_gps_imu = abs(cur_imu->timestamp - gps_timestamp);
    std::cout << "--------------------------------" << std::endl;
    std::cout << std::fixed << std::setprecision(6) << "delta_time_gps_imu: " << delta_time_gps_imu << std::endl;
    

    double delta_pose_imu_x, delta_pose_imu_y;
    delta_pose_imu_x = (last_state.G_v_I * delta_t + 0.5 * (last_state.G_R_I * acc_unbias + gravity_) * delta_t2)[0];
    delta_pose_imu_y = (last_state.G_v_I * delta_t + 0.5 * (last_state.G_R_I * acc_unbias + gravity_) * delta_t2)[1];

    std::cout << std::fixed << std::setprecision(6) << "delta_pose_imu_x: " << delta_pose_imu_x << std::endl;
    std::cout << std::fixed << std::setprecision(6) << "delta_pose_imu_y: " << delta_pose_imu_y << std::endl;
    std::cout << "--------------------------------" << std::endl;


    // if(false)
    if(delta_time_gps_imu > 5 && delta_time_gps_imu < 15 || abs(delta_pose_imu_x) > 0.02 || abs(delta_pose_imu_y) > 0.02)
    {
        state->G_p_I = last_state.G_p_I;
        ...
        Qi.block<3, 3>(0, 0) = 10 * delta_t2 * acc_noise_ * Eigen::Matrix3d::Identity();
        Qi.block<3, 3>(3, 3) = 10 * delta_t2 * gyro_noise_ * Eigen::Matrix3d::Identity();
        Qi.block<3, 3>(6, 6) = 10 * delta_t * acc_bias_noise_ * Eigen::Matrix3d::Identity();
        Qi.block<3, 3>(9, 9) = 10 * delta_t * gyro_bias_noise_ * Eigen::Matrix3d::Identity();
