...
class FusionNode {
 public:
  FusionNode(ros::NodeHandle &nh) : viewer_(nh) {
    ...
    std::string topic_imu = "/android/imu";
    std::string topic_gps = "/android/fix";
    ...
    file_state_.open("/home/touchair/test/fusion_state.txt");
  }
...
void FusionNode::gps_callback(const sensor_msgs::NavSatFixConstPtr &gps_msg) {
  ...
  std::cout << "gps timestamp: " << gps_data_ptr->timestamp << std::endl;

  std::cout << "gps cov: " << std::endl;
  std::cout << gps_data_ptr->cov << std::endl;
  ...
  *ekf_ptr_->state_ptr_ = *ekf_ptr_->state_ptr_ + K * residual;
  ...
  // save data
  {
    ...
    // file_state_ << ekf_ptr_->state_ptr_->p_wb_[0] << ", " << ekf_ptr_->state_ptr_->p_wb_[1] << ", " << ekf_ptr_->state_ptr_->p_wb_[2] << std::endl;
