   ...
   if(!is_aligned)
  {
    ...
    while(gpsifs.good())
    {
      ...
      if(line.find("R_e_w")!=string::npos)
      {
        ...
        buffer=split(line,",");
        ofs<<line<<endl;
        IMUState::R_e_w(0,0)=stod(buffer[0].c_str());
        IMUState::R_e_w(0,1)=stod(buffer[1].c_str());
        IMUState::R_e_w(0,2)=stod(buffer[2].c_str());
        getline(gpsifs,line);
        buffer=split(line,",");
        ofs<<line<<endl;
        IMUState::R_e_w(1,0)=stod(buffer[0].c_str());
        IMUState::R_e_w(1,1)=stod(buffer[1].c_str());
        IMUState::R_e_w(1,2)=stod(buffer[2].c_str());
        getline(gpsifs,line);
        buffer=split(line,",");
        ofs<<line<<endl;
        IMUState::R_e_w(2,0)=stod(buffer[0].c_str());
        IMUState::R_e_w(2,1)=stod(buffer[1].c_str());
        IMUState::R_e_w(2,2)=stod(buffer[2].c_str());
        continue;
      }
    }
    gpsifs.close();
    ...
    while(gpsdata.good())
    {
     // ofs<<"n"<<GPS_buffer.size()<<endl;
      GPS_msg gmsg;
      std::vector<std::string> buffer;
      getline(gpsdata,line);
      if(line[0]=='#') continue;
      if(line.size()<2) continue;
      buffer=split(line," ");
     // ofs<<"b"<<buffer.size()<<endl;
      if(buffer.size()<7) continue;
      gmsg.timestamp=stoll(buffer[0].c_str());
      gmsg.position=Vector3d(stod(buffer[1].c_str()),stod(buffer[2].c_str()),stod(buffer[3].c_str()));

      gmsg.velocity=Vector3d(stod(buffer[4].c_str()),stod(buffer[5].c_str()),stod(buffer[6].c_str()));
      GPS_buffer.push_back(gmsg);
      
    }
    ROS_INFO("%d",(int)(GPS_buffer.size()));
    gpsdata.close();
...
void MsckfVio::GPSCallback(const GPSMeasurementConstPtr &msg)
{
  //cout<<msg->header.stamp<<endl;
  //cout<<msg->px<<" "<<msg->py<<" "<<msg->pz<<" ";
  //cout<<endl;

}

void MsckfVio::GPSCallback()
{
  ofs<<"z"<<endl;
  if(!is_gravity_set) return;
  ofs<<"x"<<endl;
  //find current msg
  while(GPS_index<GPS_buffer.size())
  {
      //ofs<<GPS_buffer[GPS_index].timestamp/1e9-state_server.imu_state.time<<endl;
      if(GPS_buffer[GPS_index].timestamp/1e9-state_server.imu_state.time>0.001) return;
      if(GPS_buffer[GPS_index].timestamp/1e9-state_server.imu_state.time>-0.001) break;
      if(GPS_buffer.size()-1==GPS_index) return;
      GPS_index++;  
  }
  //cin.get();
  ofs<<"!"<<endl; 
...
void MsckfVio::GPSUpdate(const Eigen::MatrixXd& H,const Eigen::VectorXd&r,const Eigen::MatrixXd &noise)
{
  if (H.rows() == 0 || r.rows() == 0) return;
