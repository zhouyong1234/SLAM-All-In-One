...
class Camera {
  ...
  */
  void getRectifyMap(cv::Mat &map1,cv::Mat &map2,cv::Mat& P,double scale = 1.0) {
    if (is_fisheye_) {
      cv::fisheye::estimateNewCameraMatrixForUndistortRectify(K_, D_, cv::Size(Width,Height),
                          cv::noArray(), P, 0.0, cv::Size(Width * scale,Height * scale), 1);
      cv::fisheye::initUndistortRectifyMap(K_,D_,cv::noArray(), P,cv::Size(Width * scale, Height * scale) ,CV_32FC1, map1, map2);
    } else {
      P = cv::getOptimalNewCameraMatrix(K_, D_, cv::Size(Width,Height), 1, cv::Size(Width * scale,Height * scale));
      cv::initUndistortRectifyMap(K_, D_, cv::Mat(), P, cv::Size(Width * scale, Height * scale), CV_32FC1,map1, map2);
    }
  }
