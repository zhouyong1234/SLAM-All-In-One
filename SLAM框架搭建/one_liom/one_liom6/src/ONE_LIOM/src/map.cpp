...
void main_thread()
{
	...
	while(!odometryBuf.empty()&&!planeBuf.empty()&&!cloudBuf.empty())
	{
		...
		*laserCloudMap +=  *laserCloudPlane;
		...
		static int next_closure_Ind=0;
		//如果最近历史帧的帧数（第几帧）history_close_Ind存在
		if(history_close_Ind&&(next_closure_Ind<temp_laserCloudMap_Ind))
		{
		    next_closure_Ind=temp_laserCloudMap_Ind+20;
		    PointType center;
			...
			Eigen::Quaterniond q_w_temp(1, 0, 0, 0);
			Eigen::Vector3d t_w_temp(0, 0, 0);
			
			//位姿变换
			t_w_temp.x() = t_w_curr.x()-center.x;
			t_w_temp.y() = t_w_curr.y()-center.y;
			t_w_temp.z() = t_w_curr.z()-center.z;
			t_w_temp = t_w_temp + q_w_curr * t_icp;
			q_w_temp = q_w_curr*q_icp ;
			t_w_temp.x() = t_w_temp.x()+center.x;
			t_w_temp.y() = t_w_temp.y()+center.y;
			t_w_temp.z() = t_w_temp.z()+center.z;
	    ...
	    isam.update();
	    //获取优化后的所有位姿
	    Pose3 currEstimate;Values isamCurrentEstimate2;
	    isamCurrentEstimate2 = isam.calculateEstimate();
	    //获取当前帧的位姿，保存在当前位姿里
	    currEstimate = isamCurrentEstimate2.at<Pose3>(temp_laserCloudMap_Ind+1);
	    t_w_curr.x()=currEstimate.translation().x();
	    t_w_curr.y()=currEstimate.translation().y();
	    t_w_curr.z()=currEstimate.translation().z();
	    q_wmap_wodom = q_w_curr * q_wodom_curr.inverse();
	    t_wmap_wodom = t_w_curr - q_wmap_wodom * t_wodom_curr;
	    ...
	    //保存时间信息，用于之后gtsam优化输出
	    ros::Time time_now = ros::Time().fromSec(timeLaserCloudPlane);
	    all_time.push_back(time_now);
	    ...
	    if(temp_laserCloudMap_Ind%30==0&&temp_laserCloudMap_Ind!=0)
	    {
		//输出点云显示
...
{ 
    ...
    pubLaserGtsamPath = n.advertise<nav_msgs::Path>("/laser_gtsam_map_path", 100);
