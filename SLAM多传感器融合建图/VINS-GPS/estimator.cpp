...
bool Estimator::visualInitialAlign()
{
    ...
    vector<Vector3d> pws,pcs;

    for(int i=0;i <= frame_count; i++)
    {
        for(int j=0;j<gpsvec.size();j++)
        {
            if(abs(Headers[i].stamp.toSec()-gpsvec[j].time)<0.05)
            {
                Vector3d pw(0.0,0.0,0.0);
                if(Headers[i].stamp.toSec()>gpsvec[j].time)
                {
                    double t1=Headers[i].stamp.toSec()-gpsvec[j].time;
                ...
                {
                    double t1=Headers[i].stamp.toSec()-gpsvec[j-1].time;
    ...
    if(pws.size()<3)
    {
        ...
        {
            printf("平面对齐\n");
            if(!getRTWC2(pws,pcs,RWC,TWC,SWC))
            {
                printf("转换失败\n");
                return false;
            }
    ...
    {
        printf("空间对齐\n");
        if(!getRTWC(pws,pcs,RWC,TWC,SWC))
        {
            printf("转换失败\n");
            return false;
        }
   ...
   cout<<"**************scale***"<<SWC<<endl;
    ...
    Vector3d LBGNSS(GPS_L0,GPS_L1,GPS_L2);
...
void Estimator::optimization()
{
    ...
    for (int i = 0; i < WINDOW_SIZE + 1; i++)
    {
        ...
        problem.AddParameterBlock(para_Pose[i], SIZE_POSE, local_parameterization);//位置、姿态（四元数）7维
        problem.AddParameterBlock(para_SpeedBias[i], SIZE_SPEEDBIAS);//速度、陀螺bias、加表bias 9维
    }
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ceres::LocalParameterization *local_parameterization = new PoseLocalParameterization();
...
/*****************************************************GPS**********************************************************************/
    ...
    if (kk>0)
        options.max_solver_time_in_seconds *= 1.2;
    TicToc t_solver;
