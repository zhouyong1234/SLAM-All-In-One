...
TransformFusion::TransformFusion() {
    ...
    subLaserOdometry = nh.subscribe<nav_msgs::Odometry>("live_slam/mapping/odometry", 5, &TransformFusion::lidarOdometryHandler, this, ros::TransportHints().tcpNoDelay());
    subImuOdometry   = nh.subscribe<nav_msgs::Odometry>(odomTopic+"_incremental",   2000, &TransformFusion::imuOdometryHandler,   this, ros::TransportHints().tcpNoDelay());

    pubImuOdometry   = nh.advertise<nav_msgs::Odometry>(odomTopic, 2000);
    pubImuPath       = nh.advertise<nav_msgs::Path>    ("live_slam/imu/path", 1);
}
...
IMUPreintegration::IMUPreintegration()
{
    ...
    subOdometry = nh.subscribe<nav_msgs::Odometry>("live_slam/mapping/odometry_incremental", 5,    &IMUPreintegration::odometryHandler, this, ros::TransportHints().tcpNoDelay());
...
void IMUPreintegration::odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
{
    ...
    while (!imuQueOpt.empty())
    {
        ...
        if (imuTime < currentCorrectionTime - delta_t)
        {
            ...
            // ROS_INFO("\033[1;32m----> %f = %f - %f \033[0m", dt, imuTime, lastImuT_opt);
            // ROS_INFO("\033[1;32m----> currentCorrectionTime: %f lastImuT_opt: %f imuTime: %f dt: %f \033[0m", currentCorrectionTime, lastImuT_opt, imuTime, dt);
