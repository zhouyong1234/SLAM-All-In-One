...
class IMUEncoderFactor : public ceres::SizedCostFunction<18, 7, 9, 7, 9, 7>
{
  public:
...
///                ROS_BREAK();
            ...
            if (jacobians[4])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 7, Eigen::RowMajor>> jacobian_ex_pose(jacobians[4]);
                jacobian_ex_pose.setZero();

                jacobian_ex_pose.block<3, 3>(9, 0) = -Eigen::Matrix3d::Identity() + Qi.inverse().toRotationMatrix() * Qj.toRotationMatrix();

                jacobian_ex_pose.block<3, 3>(9, 3) = pre_integration->jacobian_ex_q;
