...
void FrameHandlerBase::setTrackingQuality(const size_t num_observations,const size_t init_match_number)
{
  ...
  if(feature_drop/init_match_number > 0.6)
  {
    ...
    SVO_WARN_STREAM("outliter: "<< feature_drop<<" init_match_number: "<<init_match_number<<" "<<feature_drop*1.0/init_match_number* 100.0<<"%");
    tracking_quality_ = TRACKING_INSUFFICIENT;
  }
}
